...
#if defined(NDEBUG)
#error "Dequant Core cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 35 * COIN;
    } else if (nHeight <= 265000 && nHeight > 250000) {
        nSubsidy = 30 * COIN;
    } else if (nHeight <= 285000 && nHeight > 265000) {
        nSubsidy = 25 * COIN;
    } else if (nHeight <= 300000 && nHeight > 285000) {
        nSubsidy = 20 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DEQ: We use certain sporks during IBD, so check to see if they are
