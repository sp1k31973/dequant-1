...
*Setup instructions for a gitian build of BHN Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BHN Core
...
- [Building BHN Core](#building-bhncore)
...
Clone the git repositories for bhncore and gitian and then checkout the bhncore version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bhncore repository
...
To build BHN Core (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bhncore repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bhncore/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bhncore/bhncore
...
For example:
```bash
URL=https://github.com/konez2k/bhncore.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bhncore=${COMMIT} --url bhncore=${URL} ../bhncore/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bhncore=${COMMIT} --url bhncore=${URL} ../bhncore/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bhncore=${COMMIT} --url bhncore=${URL} ../bhncore/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bhncore-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bhncore-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bhncore-build.assert
```
...
[bhncore/gitian.sigs](https://github.com/bhncore/gitian.sigs/) repository, or if that's not possible to create a pull
request.
